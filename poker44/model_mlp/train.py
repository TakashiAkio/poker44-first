"""Training entrypoint for the Deep Sets MLP bot detector.

    python -m poker44.model_mlp.train --num-releases 9 --epochs 60 --augment

Also exposes ``train_mlp(...)`` for programmatic use (e.g. compare.py). The
checkpoint is written to ``poker44/model_mlp/artifacts/model.pt``.
"""
...
from poker44.model_mlp.data import BatchExample, BenchmarkClient
from poker44.model_mlp.dataset import (
    BatchHandsDataset,
    Standardizer,
    collate_batches,
    split_by_release,
...
from poker44.model_mlp.metrics import evaluate, format_metrics
...
def train_mlp(
    ...
    *,
    ...
    """Train the Deep Sets MLP and return (model, standardizer, best_val_metrics)."""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Poker44 Deep Sets MLP")
    ...
    parser.add_argument("--depth", type=int, default=2, help="Per-hand MLP layers")
    ...
    extractor = HandFeatureExtractor()
    model, standardizer, best_metrics = train_mlp(
        train_ex, val_ex,
