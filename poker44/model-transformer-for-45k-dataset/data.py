"""Public benchmark API client with local caching.

Downloads Poker44 training benchmark releases and chunks from the public API
and caches raw responses on disk keyed by ``chunkHash`` for reproducibility, as
recommended in ``docs/training-benchmark.md``.

Each benchmark "chunk" object contains a ``chunks`` field that is a list of
*batches* (each a list of poker hands) plus ``groundTruth`` with one 0/1 label
per batch. The natural training example is therefore one **batch** paired with
its label.
"""
...
class BenchmarkClient:
    ...
    def status(self) -> dict:
        """Aggregate availability and latest benchmark date."""
        return self._get()
    ...
    def _hash_path(self, chunk_hash: str) -> Path:
        # chunkHash is opaque; sanitize just in case it contains path chars.
    ...
    def _load_from_index(self, index_path: Path) -> Optional[List[dict]]:
        """Reload a date's chunks from per-chunkHash cache files, if complete."""
        ...
        for h in hashes:
            ...
            if not path.exists():
                return None  # cache incomplete -> force refetch
    ...
    def fetch_chunks(
        ...
        *,
        ...
        Each chunk object is cached individually keyed by its ``chunkHash`` (per
        ``docs/training-benchmark.md``), with a per-date index listing the hashes
        so a date can be reassembled offline and shared across dates/splits.
        """
    ...
    def iter_examples(chunk_objects: Iterable[dict]) -> Iterator[BatchExample]:
        ...
        Pairs each batch in ``chunk["chunks"]`` with the aligned entry in
        ``chunk["groundTruth"]``. Chunk/label order is significant and must not
        be reordered.
        """
        for chunk in chunk_objects:
            ...
            if len(batches) != len(labels):
                # Skip malformed objects rather than misalign labels.
                continue
    ...
    def load_examples(
        ...
        *,
        ...
        for date in source_dates:
            chunk_objects = self.fetch_chunks(
                date, split=split, use_cache=use_cache
            )
