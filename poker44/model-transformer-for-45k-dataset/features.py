"""Hero-centric per-hand feature extraction.
...
The extractor turns one hand into a fixed-length float vector describing the
hero's behavior in that hand. The model aggregates these per-hand vectors across
all hands in a batch. All parsing tolerates missing/optional fields and empty
arrays, as required by the benchmark schema notes.
"""
...
_STREET_MAX_DEPTH = 4.0  # flop..showdown reachable beyond preflop
...
FEATURE_NAMES: List[str] = [
    ...
    "showdown_flag",
    # hero action counts (normalized by hero action total)
    "hero_fold_ratio",
    "hero_check_ratio",
    "hero_call_ratio",
    "hero_bet_ratio",
...
class HandFeatureExtractor:
    ...
    # bb normalization anchors (chosen from observed benchmark ranges)
    _MAX_SEATS = 9.0
    _MAX_STACK_BB = 250.0
    _MAX_BET_BB = 200.0
    _MAX_POT_BB = 250.0
    ...
    def extract(self, hand: dict) -> List[float]:
        ...
        # --- relative position within the hand's action order ---
