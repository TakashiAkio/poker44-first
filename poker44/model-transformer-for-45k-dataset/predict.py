"""Inference wrapper for the trained bot detector.

Loads a checkpoint and exposes ``predict(chunks) -> list[float]`` where
``chunks`` matches the miner-visible ``DetectionSynapse.chunks`` contract:
a list of chunk groups, each a list of hands. Returns one bot-risk score in
[0, 1] per chunk group, in order.
"""
...
from poker44.model.dataset import Standardizer
    ...
    def _load(self) -> None:
        if not self.checkpoint_path.exists():
            raise FileNotFoundError(
                ...
                "Train one with `python -m poker44.model.train`."
            )
